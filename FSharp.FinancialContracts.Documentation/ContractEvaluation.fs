...
module ContractEvaluation =
    ...
    let rec simpleEvaluateContract environment contract : Transaction list list = 
        match contract with
            | Zero               -> [[]]
            | One(currency)      -> [[Transaction(1.0,currency)]]
            | Delay(t, c)        -> listOfLength (evalTimeObs t environment) @ simpleEvaluateContract environment c
            | Scale(obs, c)      -> let subTransactions = simpleEvaluateContract environment c
                                    List.mapi (fun i transactions -> multiply transactions (evalNumberObs obs (environment |+ i))) subTransactions
            | ScaleNow(obs, c)   -> let subTransactions = simpleEvaluateContract environment c
            ...
            | And(c1, c2)        -> union (simpleEvaluateContract environment c1) (simpleEvaluateContract environment c2)
            | If(obs, t, c1, c2) -> let t' = (evalTimeObs t environment)
                                    if evalBoolObs obs environment then simpleEvaluateContract environment c1
                                    else if t' <= 0 then simpleEvaluateContract environment c2
                                    else []::(simpleEvaluateContract (environment|+1) (If(obs, TimeObs.Const(t'-1), c1, c2)))
            | Give(c)            -> let subTransactions = simpleEvaluateContract environment c
