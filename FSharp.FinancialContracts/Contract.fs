...
//        | None
    ...
    type Contract = 
      ...
      | After of IObservable<Boolean> * Contract       // Standard contract with an infinite horizon from the provided date.
      | Until of IObservable<Boolean> * Contract       // Standard ccontract with finite horizon.
      ...
      | Truncate of IObservable<Boolean> * Contract     // Contract that expires at the provided time, 
    ...
    let getTime = rx { let! x = Observable.Interval(TimeSpan.FromSeconds(1.0))
                       return DateTime.Today.AddSeconds (float x) }
    ...
    let rec eval contract (day:DateTime) = 
        [ match contract with
            | Trade(a, n) -> yield (a, n) 
            | After(dt, c) when getTime > dt -> yield! eval c day
