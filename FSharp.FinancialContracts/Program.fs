...
module program = 
    ...
    let main argv =
        let boolObservables = [(BoolVal("true"), true);
                               (BoolVal("50/50"), if ((new Random()).Next(0,2) = 0) then true else false);
                               (BoolVal("false"), false)]
        ...
        let mutable env1: Environment = 0, (boolObservables |> Map.ofList), (numObservables |> Map.ofList)
        ...
        // If vs. Or
        let c2 = Or(One(DKK), One(EUR))
        let c3 = If(BoolVal("50/50"), 3, One(DKK), One(EUR))

        let addObservables c env = 
            match env with
            | (t,boolEnv,numEnv) ->
                let (bools, nums) = getObservables c
                let newBoolEnv = List.fold (fun acc obs -> (addBoolObs (obs, true) acc)) boolEnv bools
